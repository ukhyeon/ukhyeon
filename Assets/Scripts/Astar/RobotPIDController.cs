   ...
   // float preave_q;
    ...
    void Update()
    {
       ...
       ave_r=(prepre_r+pre_r+r)/3f;
        ...
        alpha_x=(ave_p-pre_ave_p)/Time.deltaTime;
        alpha_y=(ave_q-pre_ave_q)/Time.deltaTime;
        alpha_z=(ave_r-pre_ave_r)/Time.deltaTime;
    ...
    void Move(float timer)
    {
        ...
        foreach (var wheel in wheels)
        {
            ...
            foreach (var wheel in wheels){
                wheel.wheelCollider.motorTorque =MotorTorque*Time.deltaTime;
            }
        }
    }
